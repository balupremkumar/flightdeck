...
## Progress 2026-08-11 evening (waves 5-6, commits dd3523c + wave-6)
DONE: QL-752 753 754 755 757 758 762 769 770 771 773(view-only) 774 720. Perfbudget cold-start raised 1450k->1520k deliberately (rationale in perfbudget.test.ts).
NEW BUG FILED: redactText whitespace-token pass lets KEY=sk-ant-... survive in persisted scrollback AND the existing save-scrollback export (pinned by session.test.ts:186) - fix belongs in a shared redact utility.
E2E verify list for Balu (2026-08-12): wave 1-4 list above, PLUS pwsh pane command marks/gutter/ruler + Ctrl+Up/Down, Ctrl+Shift+Space hints (copy + Shift-open), scroll-up sticky prompt, quit/relaunch scrollback restore + clean pane on Restart, subagent chip + popover on a fanned-out Claude pane, Plan-ready chip -> drawer -> Approve (sends "2"), Ctrl+Shift+R Tab deep search, Settings Diagnostics config doctor tables, Claude hooks Install (creates .bak, bell lights on approval prompt immediately), OSC 52 toggle.
Remaining QL headliners for future waves: 760 copy mode, 761 triggers, 763 output folding, 765 full categorical split, 767 permission promote-to-rule, 768 MCP board, 772 ledger polish, 775-777, 779 done, 781/783/785-791 Windows tail.

## Docker container panel (DK-1) — PLANNED 2026-08-14, parked by Balu for a future enhancement wave
Balu wants VS Code Docker-extension-style capability: a docked side panel listing containers and images, start/stop/restart/remove actions, logs, and shell-into-container.
**Full implementation plan is written and approved-in-principle: `docs/plans/docker-panel-dk1.md`** — file-level anchors, ~26 new tests, E2E checklist against real Docker Desktop. Implement from that plan, don't re-derive.
Design in one paragraph: new `src-tauri/src/docker.rs` (gitstatus.rs calm-failure pattern, every CLI call time-bounded, CREATE_NO_WINDOW only — never DETACHED_PROCESS on this machine); new `src/DockerPanel.tsx` + `src/docker.ts` (Explorer.tsx pattern: poll only while open, 5s ok / 15s daemon-down / off when no CLI); container identity rides in parameterized vendor ids `docker-exec:<ref>` / `docker-logs:<ref>` via a prefix branch in `vendors::find()` (vendors.rs:871), so Logs and Shell are ordinary panes with scrollback, search, and session restore for free, and the adapters stay out of `registry()` so they never appear in vendor pickers.
Five UI states required: no CLI / daemon not running (very common on Windows, auto-recovers) / loading / empty / populated.
